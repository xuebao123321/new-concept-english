  ...
  return (
    <div className="px-4 py-4 space-y-5">
      ...
      >
        ...
        {editingName ? (
          <input
            ...
            className="text-lg font-bold text-center bg-transparent border-b border-forest text-ink outline-none"
            autoFocus
          />
        ) : (
          <h2
            className="text-lg font-bold text-ink cursor-pointer hover:text-forest"
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: '总答题', value: totalQuestions, color: '#22D3EE' },
          { label: '总正确', value: userState.totalCorrect, color: '#10B981' },
          { label: '正确率', value: `${overallAccuracy}%`, color: '#FBBF24' },
          { label: '打卡天', value: userState.streakDays, color: '#EF4444' },
          { label: '成就', value: userState.unlockedAchievements.length, color: '#8B5CF6' },
          { label: '总XP', value: userState.totalXp, color: '#FBBF24' },
        ].map(stat => (
      ...
      <div className="glass-panel p-4">
        <h3 className="font-bold text-ink mb-3">📅 7天练习记录</h3>
          ...
          {recentStats.map((stat, i) => {
            ...
            return (
              <div key={stat.date} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[10px] font-bold text-ink-light">{stat.questionsAnswered || ''}</span>
                <motion.div
      ...
      <div className="glass-panel p-4 space-y-3">
        <h3 className="font-bold text-ink">📋 学习数据</h3>
        {[
      ...
      <div className="text-center space-y-3 pb-4">
        <button
          onClick={handleReset}
          className="text-xs text-berry/60 hover:text-berry transition-colors"
        >
          ⚠️ 重置学习数据
        </button>
        <p className="text-[10px] text-ink-muted">英语重启号 v1.0 · 熊出没·重启未来</p>
      </div>
    </div>
  );
}
